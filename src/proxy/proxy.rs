...
// /// hardcoded list of allowed for peeking hosts for now
// // let TARGET_HOSTS: [&str; 3] = ["google.com"];
...
//                 thread::spawn(move || handle_client_connection(stream, port));
//             }
//             Err(e) => {
...
// fn handle_client_traffic(mut client_stream: TcpStream, port: u16) {
...
//             // if (TARGET_HOSTS.contains(extract_host(&request_str)){
//             // }

//             if let Some(mut server_stream) = send_request_to_host(&request_str, &client_request_buffer[..bytes_read], port) {
...
// fn send_request_to_host(request_str: &str, buffer: &[u8], port: u16) -> Option<TcpStream> {
